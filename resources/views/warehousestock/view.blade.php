...
<script>
    function saveAsPDF() {
        ...
        var filename = "Warehouse Stock Requisition-" + formattedDate + ".pdf";
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-sub-header">
                        <h3 class="page-title">Warehouse Stock Information</h3>
                        <ul class="breadcrumb">
                ...
                <div class="card card-table comman-shadow">
                    <div class="card-body">
                        ...
                        <div id="printableArea" class="mt-2">
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="row mb-3 text-center align-items-center">
                                        <div class="col-sm-4 text-start">
                                            <b>Branch Name:</b> {{ $requisition->branch_name }}
                                        </div>
                                        <div class="col-sm-4">
                                            <b>Project Name:</b> {{ $requisition->project_name }}
                                        </div>
                                        <div class="col-sm-4 text-end">
                                            <b>Date:</b> {{ App\Models\Utility::dateFormat($requisition->date_from) }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($requisition->items as $key => $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->single_product_name }}</td>
                                                <td>{{ App\Models\Utility::priceFormatEng($item->price) }}</td>
                                                <td>{{ $item->demand_amount }}</td>
                                                <td>{{ App\Models\Utility::priceFormatEng($item->total_price) }}</td>
                                                <td>{{ $item->stock_level }}</td>
                                                <td>{{ App\Models\Utility::priceFormatEng($item->purchase_authorization_amount) }}</td>
                                                <td>{{ $item->comment }}</td>
                                            </tr>
                                            @php
                                                $totalPrice += $item->total_price;
                                                $totalStockLevelAmount += $item->stock_level;
                                                $totalPurchaseAuthorizationAmount += $item->purchase_authorization_amount;
                                            @endphp
                                        @endforeach
                                        <tr>
                                            <td colspan="4"></td>
                            ...
                            <div>
                                <p><b style="margin-left: 40px;">Applicant's name and signature:</b> {{$requisition->user->name}}</p>
                                    <br>
                                    <br> </br>
                                    <br> </br>
