                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="page-sub-header">
                            <h3 class="page-title">Budget Vs Actual</h3>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{ route('home') }}">Dashboard</a></li>
                                <li class="breadcrumb-item"><a href="{{route('budget.index')}}">Budget Planner</a></li>
                                ...
                                <table class="table table-bordered table-item data">
                                    ...
                                    @foreach ($incomeproduct as $productService)
                                        <tr>
                                            ...
                                            @foreach($monthList as $month)
                                                ...
                                                @endphp
                                                <td class="income_data {{$month}}_income">{{!empty (\App\Models\Utility::priceFormat($budget['income_data'][$productService->id][$month]))?\App\Models\Utility::priceFormat($budget['income_data'][$productService->id][$month]):0}}</td>
                                                <td>{{\App\Models\Utility::priceFormat($incomeArr[$productService->id][$month])}}
                                    ...
                                    <tr class="total">
                                        ...
                                        @if(!empty($budgetTotal) )
                                            @foreach($monthList as $month)
                                                ...
                                                <td class="text-dark {{$month}}_total_income"><strong>{{\App\Models\Utility::priceFormat($budgetTotal[$month])}}</strong></td>
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($incomeTotalArr[$month])}}</strong>
                                                ...
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($overBudgetTotalArr[$month]) }}</strong>
                                                    <p class="{{($budgetTotal[$month] < $overBudgetAmount)? 'text-success':''}} {{($budgetTotal[$month] > $overBudgetAmount)? 'text-danger':''}}">{{($budgetTotal[$month] !=0)? (\App\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month])!=0) ?'('.(\App\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month]).'%)') :'':''}}</p>
                                    ...
                                    @foreach ($expenseproduct as $productService)
                                        <tr>
                                            ...
                                            @foreach($monthList as $month)
                                                ...
                                                @endphp
                                                <td class="expense_data {{$month}}_expense">{{\App\Models\Utility::priceFormat(!empty($budget['expense_data'][$productService->id][$month]))?\App\Models\Utility::priceFormat($budget['expense_data'][$productService->id][$month]):0}}</td>
                                                <td>{{\App\Models\Utility::priceFormat($expenseArr[$productService->id][$month])}}
                                                ...
                                                <td>{{\App\Models\Utility::priceFormat($overBudgetAmount)}}
                                                    <p class="{{($budget['expense_data'][$productService->id][$month] < $overBudgetAmount)? 'text-success':''}} {{($budget['expense_data'][$productService->id][$month] > $overBudgetAmount)? 'text-danger':''}}" >{{($budget['expense_data'][$productService->id][$month] !=0)? (\App\Models\Budget::percentage($budget['expense_data'][$productService->id][$month],$overBudgetAmount) !=0) ?'('.(\App\Models\Budget::percentage($budget['expense_data'][$productService->id][$month],$overBudgetAmount).'%)') :'':''}}</p>
                                    ...
                                    <tr class="total">
                                        ...
                                        @if(!empty($budgetExpenseTotal) )
                                            @foreach($monthList as $month)
                                                <td class="text-dark {{$month}}_total_expense"><strong>{{\App\Models\Utility::priceFormat($budgetExpenseTotal[$month])}}</strong></td>
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($expenseTotalArr[$month])}}</strong>
                                                ...
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($overExpenseBudgetTotalArr[$month]) }}</strong>
                                                    <p class="{{($budgetExpenseTotal[$month] < $overExpenseBudgetTotalArr[$month])? 'text-success':''}} {{($budgetExpenseTotal[$month] >$overExpenseBudgetTotalArr[$month])? 'text-danger':''}}">{{($budgetExpenseTotal[$month] !=0)? (\App\Models\Budget::percentage($budgetExpenseTotal[$month],$overExpenseBudgetTotalArr[$month])!=0) ? '('.(\App\Models\Budget::percentage($budgetExpenseTotal[$month],$overExpenseBudgetTotalArr[$month]).'%)') :'':''}}</p>
                                    ...
                                    <tr class="total">
                                        ...
                                        @if(!empty($budgetprofit) )
                                            ...
                                            @foreach($monthList as $month)
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($budgetprofit[$month]) }}</strong></td>
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($actualprofit[$month]) }}</strong>
                                                ...
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($overbudgetprofit[$month]) }}</strong>
                                                    <p class="{{($budgetprofit[$month] < $overbudgetprofit[$month])? 'text-success':''}} {{($budgetprofit[$month] < $overbudgetprofit[$month])? 'text-success':''}}">{{($budgetprofit[$month] !=0)? (\App\Models\Budget::percentage($budgetprofit[$month],$overbudgetprofit[$month])!=0) ? '('.(\App\Models\Budget::percentage($budgetprofit[$month],$overbudgetprofit[$month]).'%)') :'':''}}</p>
                                ...
                                <table class="table table-bordered table-item data">
                                    ...
                                    @foreach ($incomeproduct as $productService)
                                        <tr>
                                            ...
                                            @foreach($quarterly_monthlist as $month)
                                                ...
                                                <td>{{\App\Models\Utility::priceFormat($overBudgetAmount)}}
                                                    <p class="{{($budget['income_data'][$productService->id][$month] < $overBudgetAmount)? 'text-success':''}} {{($budget['income_data'][$productService->id][$month] > $overBudgetAmount)? 'text-danger':''}}">{{($budget['income_data'][$productService->id][$month] !=0)? '('.(\App\Models\Budget::percentage($budget['income_data'][$productService->id][$month],$overBudgetAmount).'%)') :''}}</p>
                                                </td>
                                    ...
                                    <tr class="total">
                                        ...
                                        @if(!empty($budgetTotal) )
                                            ...
                                            @foreach($quarterly_monthlist as $month)
                                                ...
                                                <td class="text-dark {{$month}}_total_income"><strong>{{\App\Models\Utility::priceFormat($budgetTotal[$month])}}</strong></td>
                                                <td><strong>{{\App\Models\Utility::priceFormat($incomeTotalArr[$month])}}</strong>
                                                    <p>{{($budgetTotal[$month] !=0)? (\App\Models\Budget::percentage($budgetTotal[$month],$incomeTotalArr[$month]) !=0)?'('.(\App\Models\Budget::percentage($budgetTotal[$month],$incomeTotalArr[$month]).'%)') :'':''}}</p>
                                                </td>
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($overBudgetTotalArr[$month]) }}</strong>
                                                    <p class="{{($budgetTotal[$month] < $overBudgetTotalArr[$month])? 'text-success':''}} {{($budgetTotal[$month] > $overBudgetTotalArr[$month])? 'text-danger':''}}">{{($budgetTotal[$month] !=0)? '('.(\App\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month]).'%)') :''}}</p>
                                                </td>
                                            @endforeach
                                        @endif
                                    ...
                                    <tr class="total">
                                        ...
                                        @if(!empty($budgetprofit) )
                                            @foreach($quarterly_monthlist as $month)
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($budgetprofit[$month])}}</strong></td>
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($actualprofit[$month]) }}</strong></td>
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($overbudgetprofit[$month]) }}</strong>
                                                    <p class="{{($budgetprofit[$month] < $overbudgetprofit[$month])? 'text-success':''}} {{($budgetprofit[$month] > $overbudgetprofit[$month])? 'text-danger':''}}">{{($budgetprofit[$month] !=0)? (\App\Models\Budget::percentage($budgetprofit[$month],$overbudgetprofit[$month])!=0) ? '('.(\App\Models\Budget::percentage($budgetprofit[$month],$overbudgetprofit[$month]).'%)') :'':''}}</p>
                                                </td>
                                            @endforeach
                                        @endif
                                    ...
                                    <tr class="total">
                                        ...
                                        @if(!empty($budgetTotal) )
                                            @foreach($half_yearly_monthlist as $month)
                                                ...
                                                <td class="text-dark"><strong>{{\App\Models\Utility::priceFormat($overBudgetTotalArr[$month]) }}</strong>
                                                    <p class="{{($budgetTotal[$month] < $overBudgetTotalArr[$month])? 'text-success':''}} {{($budgetTotal[$month] > $overBudgetTotalArr[$month])? 'text-danger':''}}">{{($budgetTotal[$month] !=0)? (\App\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month])!=0) ?'('.(\App\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month]).'%)') :'':''}}</p>
