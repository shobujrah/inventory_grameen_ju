...
<script>
    function saveAsPDF() {
        ...
        var filename = "Completed Order Requisition-" + formattedDate + ".pdf";
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-sub-header">
                        <h3 class="page-title">Completed Order View</h3>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('home') }}">Dashboard</a></li>
                            <li class="breadcrumb-item active">Completed Order View</li>
                        </ul>
                    </div>
                </div>
            </div>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($requisitionlist as $key => $requisition)
                                            <tr>
                                                ...
                                                <td class="text-center">{{ $requisition->demand_amount }}</td>
                                                <td>{{ App\Models\Utility::priceFormatEng($requisition->total_price) }}</td>
                                                <td>{{ $requisition->comment }}</td>
                                                <td class="text-center">
                                                    @if ($requisition->delivery == 1 && $requisition->stock_status == 0)
                                                        <span class="badge" style="background-color: #28a745; color: #fff; padding: 0.25em 0.5em; border-radius: 0.25rem; font-weight: 600;">
                                                            Delivered
                                                        </span>
                                                    @elseif ($requisition->purchase == 1)
                                                        <span class="badge" style="background-color: #17a2b8; color: #fff; padding: 0.25em 0.5em; border-radius: 0.25rem; font-weight: 600;">
                                                            Progress in Purchase
                                                        </span>
                                                    @elseif ($requisition->purchase == 2)
                                                            <span class="badge" style="background-color: #fd7e14; color: #fff; padding: 0.25em 0.5em; border-radius: 0.25rem; font-weight: 600;">
                                                                Purchased
                                                            </span>
                                                    @elseif ($requisition->delivery == 0 && $requisition->purchase == 0 && $requisition->reject == 0)
                                                        <span class="badge" style="background-color: #ffc107; color: #fff; padding: 0.25em 0.5em; border-radius: 0.25rem; font-weight: 600;">
                                                            Pending
                                                        </span>
                                                    @elseif ($requisition->reject == 1)
                                                        <span class="badge" style="background-color: #dc3545; color: #fff; padding: 0.25em 0.5em; border-radius: 0.25rem; font-weight: 600;">
                                                            Rejected
                                                        </span>

                                                    @elseif ($requisition->delivery == 1 && $requisition->stock_status == 1)
                                                        <span class="badge" style="background-color: #007bff; color: #fff; padding: 0.25em 0.5em; border-radius: 0.25rem; font-weight: 600;">
                                                            Stocked
                                                        </span>
