...
class ServeCommand extends Command
{
    ...
    protected function getDateFromLine($line)
    {
        ...
        if (preg_match($regex, $line, $matches)) {
            try {
                ...
                throw new \Exception("Error parsing date from line: " . $e->getMessage());
            }
        } else {
            
            throw new \Exception("Invalid log line format: $line");
        }
    ...
    protected function getRequestPortFromLine($line)
    {
        if (preg_match('/:(\d+)\s(?:(?:\w+$)|(?:\[.*))/', $line, $matches)) {
            ...
            throw new \Exception("Unable to extract port from line: $line");
        }
    }
